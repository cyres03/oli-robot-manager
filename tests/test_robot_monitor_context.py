...
def _robot_info_data(version="v1", sn="WF_TRON2A_185"):
    return {
        "result": [{
            "name": "system_info",
            "values": [
...
def test_monitor_drops_status_from_old_target_generation(qapp):
    ...
    monitor.update_target("ws://new:5000", "WF_TRON2A_185")
    statuses = []
...
def test_monitor_emits_current_target_context(qapp):
    monitor = RobotMonitor()
    monitor.update_target("ws://robot:5000", "WF_TRON2A_185")
    ...
    assert statuses[0]["_target_accid"] == "WF_TRON2A_185"
    assert statuses[0]["_reported_accid"] == "WF_TRON2A_185"
