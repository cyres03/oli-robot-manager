...
def test_default_oli_health_check_does_not_repair_cpu(monkeypatch):
    service = HealthCheckService()
    monkeypatch.setattr(service, "_check_wifi", lambda: None)
    service.run_full_diagnostic(allow_repairs=False)
    repairs = []
    next_steps = []
    monkeypatch.setattr(ROBOT_CONFIG, "expected_cpu_cores", 8)
    monkeypatch.setattr(ROBOT_CONFIG, "allow_cpu_repair", True)
    monkeypatch.setattr(service, "_fix_cpu", lambda: repairs.append(True))
    monkeypatch.setattr(service, "_check_camera", lambda: next_steps.append("camera"))
...
def test_default_oli_health_check_does_not_repair_time(monkeypatch):
    service = HealthCheckService()
    monkeypatch.setattr(service, "_check_wifi", lambda: None)
    service.run_full_diagnostic(allow_repairs=False)
    repairs = []
    next_steps = []
    monkeypatch.setattr(ROBOT_CONFIG, "allow_time_repair", True)
    monkeypatch.setattr(service, "_fix_time", lambda: repairs.append(True))
    monkeypatch.setattr(service, "_check_imu", lambda: next_steps.append("imu"))
