...
- `HU_D04_01_*`（Oli）：保留现有控制、验收和校零能力。
- `HU_L04_01_*`（Luna L04）：已接入身份、状态、`.2/.4` 节点、日志和只读查询；
	运动、动作执行、校零、Backlash、灯效和语音设置在专项真机验收前保持锁定。

软件根据已连接的机器人 WiFi 和 8080 返回的 SN 自动选择 Profile。身份不一致、
未知型号或同时连接多台机器人时不会回退到默认 Oli 目标。
...
- [HU_L04_01 接口兼容性调研报告](HU_L04_01接口兼容性调研报告.md)
